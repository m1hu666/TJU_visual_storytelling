...
def parse_args():
    p = argparse.ArgumentParser(
        description='Resize all images in a folder to match the size of a target image (Building_Tiannan.png).')
    default_dir = Path(__file__).resolve().parent / 'raw_picture'
    default_target = Path(r"D:/Course/Information visualization/final_project/Weijinlu_Campus/raw_picture/Building_Tiannan.png")
    ...
    p.add_argument('--inplace', action='store_true',
                   help='If set, overwrite original images. Otherwise save to a subfolder `resized_by_Building_Tiannan`.')
...
def main():
    ...
    if args.inplace:
        ...
        out_dir = src_dir / 'resized_by_Building_Tiannan'
